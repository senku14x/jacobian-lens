...
Items:
  ...
  nulls            : per primary phrase, NULLS_PER surprisal-matched 2-token non-name bigrams (suffix log p within +-0.3 nats of
                     ...
                     plus one shared junk pair
Objectives per token (batch 1, retained graph): logp, lin, logit, odds. Both reductions (at t', source-mean) saved per context.
Checkpointed per item: outputs/003a/grads/<item>__<regime>.pt; resumes by skipping existing files.
...
natural_docs = set(json.load(open(f"{RES1}/natural_doc_ids.json")))
sigma_docs = set(json.load(open(f"{RES1}/sigma_meta.json"))["seq_ids"])
nat_ctx = {}
...
def phrase_logps(text_ids, pids):
    ...
    from lib.ekko_harness import capture
...
for nc in NULL_CANDS:
    ids = tok.encode(" " + nc, add_special_tokens=False)
    if len(ids) == 2: null_cands[nc] = ids
...
for ph in PRIMARY:
    ...
    for nc, lp in ranked[:NULLS_PER]:
        items.append((f"null:{nc}", null_cands[nc], 1, f"null_for:{ph}", "natural", cs))
